...
function doSearch_work(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#workList").datagrid({
			...
			nowrap:true, toolbar:"toolbar_work", url:'others/list', method:'get', loadMsg:'数据加载中......',
		...
		$("#workList").datagrid({  
	        title:'作业列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get',
			nowrap:true, toolbar:"toolbar_work", url:'others/search_work_by_'+name+'?searchValue='+value,
    ...
    function work_add(){
    	$.get("others/add_judge",'',function(data){
       		if(data.msg != null){
       			$.messager.alert('提示', data.msg);
       		}else{
    ...
    function work_edit(){
    	$.get("others/edit_judge",'',function(data){
       		if(data.msg != null){
       			$.messager.alert('提示', data.msg);
       		}else{
    ...
    function work_delete(){
    	$.get("others/delete_judge",'',function(data){
       		if(data.msg != null){
       			$.messager.alert('提示', data.msg);
       		}else{
       			var ids = getWorkSelectionsIds();
            	...
            	$.messager.confirm('确认','确定删除ID为 '+ids+' 的作业吗？',function(r){
            	    if (r){
            	    	var params = {"ids":ids};
                    	$.post("others/delete_batch",params, function(data){
                			if(data.status == 200){
